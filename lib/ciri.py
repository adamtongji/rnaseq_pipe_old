...
def ciri_run(outprefix,genomefile,annotationfile, *files):
    if len(files)!=2:
        ...
        sys.exit(1)
    # for _file in files:
    #     if _file.endswidh(".gz"):
    #         sh("gzip -d {0} >{1}.fastq".format(_file, outprefix))
    #     else:
    #         sh("cp {0} {1}.fastq".format(_file, outprefix))
...
def ciri_process(outputdir,nrep, pairtype, pvalue, genomefile,genome="hg19"):
